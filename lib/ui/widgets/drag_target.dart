...
class Draggable<T extends Object> extends StatefulWidget {
  ...
  final DragStartCallback<T>? onDragStarted;
...
class _DraggableState<T extends Object> extends State<Draggable<T>> {
  ...
  DragAvatar<T>? _startDrag(Offset position) {
    ...
    final newAvatar = widget.onDragStarted == null
        ? avatar
        : widget.onDragStarted?.call(avatar);
    if (newAvatar != null) {
      setState(() {
        _activeCount += 1;
      });
...
class DragAvatar<T extends Object> extends Drag {
  ...
  void end(DragEndDetails details) {
    finishDrag(DragEndKind.dropped, _restrictVelocityAxis(details.velocity));
  }
